...
export default class CrossAJoystick extends Joystick {
    ...
    constructor(game, parent) {
        ...
        new Button(this.game, cross, Keyboard.LEFT, 0, 0, 'joystick_cross', {angle: 0});
        new Button(this.game, cross, Keyboard.UP, 0, 0, 'joystick_cross', {angle: 90});
        new Button(this.game, cross, Keyboard.RIGHT, 0, 0, 'joystick_cross', {angle: 180});
        new Button(this.game, cross, Keyboard.DOWN, 0, 0, 'joystick_cross', {angle: 270});
        cross.forEach((b) => b.anchor.setTo(1, 1));
        cross.angle = -45;
        cross.scale.set(this.game.uiScale(Joystick.SCALE));
        cross.x = cross.width/2 + Joystick.MARGIN;
        cross.y = this.game.canvas.height - cross.height/2 - Joystick.MARGIN;
        this.add(cross);
