...
export default class TremisProtectQuest extends Quest {
    ...
    constructor(game) {
        ...
        game.materialGroup.forEach((material) => {
            for (const protect of Config.containers[Config.containerIndex['soubassement']].protects) {
    ...
    onProtect(material) {
        let isFinished = true;
        if (Config.containers[Config.containerIndex['soubassement']].isProtected === false) {
            isFinished = false;
        }
        if (isFinished) {
